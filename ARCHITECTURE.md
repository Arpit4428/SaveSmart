...
- **The Financial Engine** (Python) handles 100% of arithmetic, simulation, cascade modeling, resilience scoring, and recovery plan optimization in INR (₹).
...
- **The Frontend** (Next.js 15) renders responsive UI components, interactive charts (Recharts), and manages user interaction with Indian Rupee (₹) formatting.
...
2. **Context-Only Ingestion:** Gemini receives structured JSON containing already computed results in INR (₹):
   - `resilience_score`: e.g., `42.5`
   - `baseline_completion_month`: e.g., `18`
   - `stressed_completion_month`: e.g., `26`
   - `slippage_months`: e.g., `8`
   - `total_deficit_rupees`: e.g., `250000.00`
...
4. **Post-Processing Guardrail Check:** A lightweight Python validator ensures the LLM's narrative contains no currency amounts (₹ / INR) or numbers that conflict with the source calculation payload.
...
├── README.md                       # Project README and quick start (INR reference)
├── PROJECT_STATE.md                # Living status and milestone tracker
├── ARCHITECTURE.md                 # System architecture (this document)
├── API_CONTRACT.md                 # Full OpenAPI and endpoint schema spec (₹ standard)
...
│   │   │
│   │   ├── engine/                 # PURE DETERMINISTIC FINANCIAL MATH (INR / ₹)
...
└── frontend/                       # Next.js 15 Client Application
    ...
    └── src/
        ...
        │   └── utils.ts            # Formatting (INR currency formatting, date, percentage)
        │
        └── types/                  # Shared TypeScript Interfaces (Mirrors Pydantic)
            ├── api.ts              # Generic API response shapes (INR numbers)
...
### 4.2. Document Schema Specifications (Values in INR / ₹)
...
```json
{
  ...
  "results": {
    ...
    "minimum_liquidity_amount": -45000.00,
...
|                     | backend/tests/test_*.py       | Math tests only (INR precision)          |
+---------------------+-------------------------------+------------------------------------------+
...
|                     | frontend/src/lib/             | api-client.ts, utils.ts (₹ formatter)    |
+---------------------+-------------------------------+------------------------------------------+
...
User clicks [Run Stress Test]
    ...
    --> FastAPI validates request payload against ShockRequest schema
      --> SimulationService loads Baseline & Goal from MongoDB
        --> FinancialEngine.simulate_shocks(baseline, goal, shocks)
            - Step 1: Compute month-by-month cashflow in INR
  ...
  --> Frontend requests POST /api/v1/explain/scenario (passes verified JSON)
    --> GeminiExplainer generates human narrative strictly referencing figures in ₹
...
1. **Deterministic Engine Independence:** The financial engine has zero external network calls. If MongoDB or Gemini are unavailable, the engine executes in-memory.
2. **Gemini Fallback:** If Gemini API times out or rate limits (`429`), the backend returns a deterministic fallback template summary generated via Python rule-based strings formatted in ₹ so the user is never blocked.
