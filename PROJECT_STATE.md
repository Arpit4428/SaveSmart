...
1. **Model** realistic goals, baseline cash flows, fixed commitments, and discretionary buffers.
...
| **F-02** | **Financial Baseline** | Track net income, fixed obligations, discretionary spending, emergency savings, and debt liabilities. | `backend/app/api/v1/baseline.py`<br>`frontend/src/app/baseline/` | ?? Planned (Contracts Ready) | Backend + Financial Engine |
...
| **F-07** | **Goal Survival Map** | Multi-month timeline projection showing cash balances, safety buffer, and insolvency thresholds. | `backend/app/engine/survival.py`<br>`frontend/src/app/survival/` | ?? Planned (Math Spec Ready) | Financial Engine + Frontend |
| **F-08** | **Cascade Mode** | Simulates compounding shocks in sequence (e.g. Month 2 Job Loss + Month 4 Car Breakdown). | `backend/app/engine/cascade.py`<br>`frontend/src/components/stress-test/` | ?? Planned (Math Spec Ready) | Financial Engine |
...
```
          ...
          |  (Dollar amounts, months,   |                       |  (Goals, baseline, runs)    |
          |   scores, recovery steps)   |                       +-----------------------------+
          +--------------+--------------+
                         |
                         | (Read-only verified input context)
                         v
          +-----------------------------+
...
> **Strict Rule**: Gemini is treated strictly as an **explainer and narrator**, never a calculator. Gemini receives pure JSON payloads containing verified mathematical values produced by the deterministic Python financial engine. Gemini's system instructions explicitly forbid generating or modifying numbers.
...
1. **Workstream A: Financial Engine Core (`/backend/app/engine/`)**
   - Independent pure Python logic with pytest suites. No web framework or DB dependencies.
...
3. **Workstream C: Gemini Explainer Layer (`/backend/app/services/gemini_explainer.py`)**
   - Prompt engineering, guardrail schema verification, structured Markdown output generation.
4. **Workstream D: Frontend UI & Visualization (`/frontend/`)**
   - Next.js 15, Recharts, Tailwind CSS, typed API client matching `API_CONTRACT.md`.
5. **Workstream E: QA, Verification & Benchmarks (`/backend/tests/`)**
   - Math regression tests, edge-case shock scenarios, API contract tests.
